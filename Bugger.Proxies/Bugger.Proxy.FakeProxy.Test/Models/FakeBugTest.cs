...
namespace Bugger.Proxy.FakeProxy.Test.Models
{
    [TestClass]
    public class TFSBugTest
    {
        ...
        {
            var defaultItem = new FakeBug();
            Assert.AreEqual(BugType.Yellow, defaultItem.Type);

            var item = new FakeBug()
            {
                ID = 123,
                Title = "Bug A",
        ...
        public void CheckUpdateTest()
        {
            ...
            AssertHelper.ExpectedException<ArgumentException>(() => item3.CheckUpdate(item1));

            item3.CheckUpdate(item2);
